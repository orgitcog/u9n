...
void AtomTable::clear() {
    ...
    node_data_.clear();
    link_data_.clear();
    incoming_sets_.clear();
    hash_index_.clear();
    free_slots_.clear();
    atom_count_.store(0, std::memory_order_relaxed);
    node_count_.store(0, std::memory_order_relaxed);
    link_count_.store(0, std::memory_order_relaxed);
}
...
bool AtomTable::remove_atom(AtomId id, bool recursive) {
    ...
    std::unique_lock lock(global_mutex_);
    return remove_atom_unlocked(id, recursive);
}

// Precondition: caller holds global_mutex_ exclusively.
// Needed because recursive removal would self-deadlock on the
// non-recursive shared_mutex if it re-entered remove_atom().
bool AtomTable::remove_atom_unlocked(AtomId id, bool recursive) {
    if (!is_valid_slot(id)) return false;
    ...
    if (!incoming_sets_[slot].empty()) {
        ...
        for (AtomId link_id : incoming) {
            remove_atom_unlocked(link_id, true);
        }
    }
