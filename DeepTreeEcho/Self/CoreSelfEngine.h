...
/** Slim telemetry for identity updates — avoids a circular include with AutonomyPipeline. */
struct FCoreSelfTelemetry
{
    float AARCoherence = 0.0f;
    float SelfModelAccuracy = 0.0f;
    float EmotionalValence = 0.0f;
    int32 AutognosisLevel = 0;
    int32 AutonomyLevel = 0;
...
public:
     ...
     */
    void Update(const FCoreSelfTelemetry& Telemetry)
    {
        check(bInitialized);
        CycleCount++;
    ...
    const TArray<FIdentityNode>& GetIdentityNodes() const { return Nodes; }
    const TArray<FIdentityEdge>& GetIdentityEdges() const { return Edges; }
...
private:
    ...
    void UpdateGeneExpressions(const FCoreSelfTelemetry& Telemetry)
    {
    ...
    void UpdateAttention(const FCoreSelfTelemetry& Telemetry)
    {
    ...
    void CheckStageAdvancement(const FCoreSelfTelemetry& Telemetry)
    {
        switch (Stage)
        {
        ...
        case EOntogeneticStage::NEONATAL:
            if (Telemetry.AutognosisLevel >= 1 && CycleCount > 500)
        ...
        case EOntogeneticStage::JUVENILE:
            if (Telemetry.AutonomyLevel >= 2 && CycleCount > 5000)
                Stage = EOntogeneticStage::ADOLESCENT;
            break;
        case EOntogeneticStage::ADOLESCENT:
            if (Telemetry.AutonomyLevel >= 3 && CycleCount > 10000)
