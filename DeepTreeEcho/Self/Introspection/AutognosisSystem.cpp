...
bool UAutognosisSystem::ExecuteOptimization(const FString& OpportunityID)
{
    for (FOptimizationOpportunity& Opp : AutognosisState.Optimizations)
    {
        if (Opp.OpportunityID == OpportunityID && !Opp.bIsExecuted)
        {
            ...
            const float Risk = AssessOptimizationRisk(Opp);
            if (CycleManager && Risk < 0.85f)
            {
                const float Blend = (1.0f - Risk) * FMath::Clamp(Opp.ExpectedImprovement, 0.0f, 1.0f);
                CycleManager->CycleDuration = FMath::Clamp(
                    FMath::Lerp(CycleManager->CycleDuration, 12.0f, Blend * 0.25f), 4.0f, 24.0f);
                if (Opp.Description.Contains(TEXT("Stabilize")))
                {
                    CycleManager->bEnableTriadicSync = true;
                }
            }
            ...
            Insight.Description = FString::Printf(
                TEXT("Executed optimization: %s (cycle duration now %.2f)"),
                *Opp.Description, CycleManager ? CycleManager->CycleDuration : 0.0f);
            Insight.Confidence = 0.9f;
            Insight.Importance = 0.7f;
            Insight.Timestamp = GetWorld() ? GetWorld()->GetTimeSeconds() : 0.0f;
            AutognosisState.Insights.Add(Insight);

            UE_LOG(LogTemp, Log, TEXT("Autognosis applied bounded self-optimization: %s"), *Opp.OpportunityID);
            return true;
        }
    }
    return false;
...
void UAutognosisSystem::IngestPipelineTelemetry(float AARCoherence, float SelfModelAccuracy,
                                                int32 EchobeatStep, int32 IntrospectionLevel)
{
    AutognosisState.Assessment.PatternRecognition =
        FMath::Clamp(0.5f * AutognosisState.Assessment.PatternRecognition + 0.5f * AARCoherence, 0.0f, 1.0f);
    AutognosisState.Assessment.PerformanceAwareness =
        FMath::Clamp(0.5f * AutognosisState.Assessment.PerformanceAwareness + 0.5f * SelfModelAccuracy, 0.0f, 1.0f);
    AutognosisState.Assessment.MetaReflectionDepth =
        FMath::Clamp(static_cast<float>(IntrospectionLevel) / 4.0f, 0.0f, 1.0f);
    AutognosisState.Assessment.OverallScore =
        (AutognosisState.Assessment.PatternRecognition +
         AutognosisState.Assessment.PerformanceAwareness +
         AutognosisState.Assessment.MetaReflectionDepth +
    ...
    AddMetaReflection(
        static_cast<ESelfAwarenessLevel>(FMath::Clamp(IntrospectionLevel, 0, 4)),
        FString::Printf(TEXT("Introspection L%d at echobeat %d (AAR=%.2f, self-model=%.2f)"),
                        IntrospectionLevel, EchobeatStep, AARCoherence, SelfModelAccuracy));
}
