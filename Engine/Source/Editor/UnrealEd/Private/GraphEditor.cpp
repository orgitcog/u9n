...
void SGraphEditor::UpgradeDeprecatedDelegates(FGraphEditorEvents& EventsToUpdate)
{
	...
	{
		EventsToUpdate.OnDropActors = FOnDropActors::CreateLambda([OnDropActorCopy = EventsToUpdate.OnDropActor](const TArray< TWeakObjectPtr<AActor> >& Actors, UEdGraph* InGraph, const FVector2f& InDropLocation)
		{
			OnDropActorCopy.Execute(Actors, InGraph, FVector2D(InDropLocation));
		});
	}
	if (EventsToUpdate.OnDropStreamingLevel.IsBound() && !EventsToUpdate.OnDropStreamingLevels.IsBound())
	{
		EventsToUpdate.OnDropStreamingLevels = FOnDropStreamingLevels::CreateLambda([OnDropStreamingLevelCopy = EventsToUpdate.OnDropStreamingLevel](const TArray< TWeakObjectPtr<class ULevelStreaming> >& Levels, UEdGraph* InGraph, const FVector2f& InDropLocation)
		{
			OnDropStreamingLevelCopy.Execute(Levels, InGraph, FVector2D(InDropLocation));
		});
	}
	if (EventsToUpdate.OnCreateActionMenu.IsBound() && !EventsToUpdate.OnCreateActionMenuAtLocation.IsBound())
	{
		EventsToUpdate.OnCreateActionMenuAtLocation = FOnCreateActionMenuAtLocation::CreateLambda([OnCreateActionMenuCopy = EventsToUpdate.OnCreateActionMenu](UEdGraph* InGraph, const FVector2f& InLocation, const TArray<UEdGraphPin*>& InPins, bool bAutoExpand, FActionMenuClosed InMenuClosed)
		{
			return OnCreateActionMenuCopy.Execute(InGraph, FVector2D(InLocation), InPins, bAutoExpand, InMenuClosed);
		});
	}
	if (EventsToUpdate.OnSpawnNodeByShortcut.IsBound() && !EventsToUpdate.OnSpawnNodeByShortcutAtLocation.IsBound())
	{
		EventsToUpdate.OnSpawnNodeByShortcutAtLocation = FOnSpawnNodeByShortcutAtLocation::CreateLambda([OnSpawnNodeByShortcutCopy = EventsToUpdate.OnSpawnNodeByShortcut](FInputChord InInputChord, const FVector2f& InLocation)
		{
			return OnSpawnNodeByShortcutCopy.Execute(InInputChord, FVector2D(InLocation));
		});
	}
