...
void FBehaviorTreeDebugger::Tick(float DeltaTime)
{
	...
	if ((!TreeInstance.IsValid() || !RootNode.IsValid())
		&& !IsPlaySessionPaused())
	{
	...
	if (DebuggerInstanceIndex != INDEX_NONE)
	{
		...
		if (UBTNode* InstanceRootNode = ShowInstance.RootNode.Get(); IsValidChecked(InstanceRootNode))
		{
			...
			if (ActiveStepIndex == LatestStepIndex)
			{
				...
				TreeInstance->StoreDebuggerRuntimeValues(RuntimeDescriptions, InstanceRootNode, IntCastChecked<uint16>(DebuggerInstanceIndex));
