...
{
	// Array of pairs - possible folder identifier and bool value indicating whether Content folder should be appended to BaseFolderName
	// For example, we want to move contents of __ExternalActors__/ folder to Content/__ExternalActors__/
	TStaticArray<TPair<FString, bool>, 3> FolderIdents;
	FolderIdents[0] = { TEXT("__ExternalActors__/"), true };
	FolderIdents[1] = { TEXT("__ExternalObjects__/"), true };
	FolderIdents[2] = { TEXT("Content/"), false };
	...
	for (int32 iFile = 0; iFile < FilesToAdd.Num(); ++iFile)
	{
		...
		int32 BaseFolderNameIndex = INDEX_NONE;
		int32 BaseFolderNameLength = 0;
		bool bAppendContentFolderToBaseFolderName = false;

		for (const TPair<FString, bool>& FolderIdentPair : FolderIdents)
		{
			if (int32 FolderNameIndex = EachFile.Find(FolderIdentPair.Key); FolderNameIndex != INDEX_NONE)
			{
				BaseFolderNameIndex = FolderNameIndex;
				BaseFolderNameLength = FolderIdentPair.Key.Len();
				bAppendContentFolderToBaseFolderName = FolderIdentPair.Value;
				break;
			}
		}

		if (BaseFolderNameIndex != INDEX_NONE)
		{
			FString ContentFile = EachFile.RightChop(BaseFolderNameIndex);
			...
			{
				ContentFile.InsertAt(BaseFolderNameLength - 1, GameFolder);
			}

			if (bAppendContentFolderToBaseFolderName)
			{
				ContentFile = TEXT("Content/") + ContentFile;
			}
