...
void FPropertyValueImpl::EnumerateObjectsToModify(FPropertyNode* InPropertyNode, const EnumerateObjectsToModifyFuncRef& InObjectsToModifyCallback) const
{
	...
	// If it exists, we might have a complex struct owned by an object, so we try to extract the object address.
	...
	if (ComplexNode)
	{
		...
		const FObjectPropertyNode* ObjectNode = bIsStruct ? InPropertyNode->FindObjectItemParent() : nullptr;
		const int32 NumObjectInstances = ObjectNode ? ObjectNode->GetInstancesNum() : 0;
		for (int32 Index = 0; Index < NumInstances; ++Index)
		{
			...
			if (bIsStruct)
			{
				...
				// Make sure we have the same number of structs and objects, so there is a 1 for 1 mapping between the struct and its owning object.
				// If we don't, we don't have a clear way to resolve this, so we won't try to retrieve the object address (which was the previous behavior).
				if (ObjectNode && NumInstances == NumObjectInstances)
				{
