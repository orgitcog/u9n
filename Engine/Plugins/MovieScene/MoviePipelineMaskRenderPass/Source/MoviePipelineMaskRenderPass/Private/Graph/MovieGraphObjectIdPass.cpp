...
{
	UE::MoviePipeline::FObjectIdAccelerationData* AccelerationData = GetAccelerationData(InSampleState.TraversalContext.RenderDataIdentifier.RootBranchName);
	check(AccelerationData);

	// Update the HitProxy acceleration cache; this needs to be updated every frame. It's important to call this AFTER the render finishes because
	// HitProxy IDs will be out-of-date before the render starts. Render property changes made during the frame (via modifiers) will invalidate the
	// HitProxy IDs in the global array, and those invalidations happen only once Render() is called.
	//
	// Doing the update here is the best spot as far as timing goes. At this point, the render has finished, modifiers are still applied, and the next
	// layer's render has not started yet, which ensures that the global HitProxy array is still reflective of what was just rendered.
	{
		const UMovieGraphObjectIdNode* ParentNode = Cast<UMovieGraphObjectIdNode>(GetParentNode(InSampleState.TraversalContext.Time.EvaluatedConfig));
		check(ParentNode);

		UpdateManifestAccelerationData(*AccelerationData, ParentNode->IdType);
	}
