...
void UMovieGraphGlobalGameOverridesNode::ApplySettings(const bool bOverrideValues, UWorld* InWorld)
{
	...
	MOVIEPIPELINE_STORE_AND_OVERRIDE_CVAR_BOOL(NodeCDO->PreviousAlphaOutput, TEXT("r.PostProcessing.PropagateAlpha"), UE::MoviePipeline::GetAlphaOutputOverride(), bOverrideValues);
	...
	// user needing to customize these (or be aware of them) is very low.
	{
		...
		if (VTInvalidateCvar && VTInvalidateCvar->GetInt() != 0)
		{
			MOVIEPIPELINE_STORE_AND_OVERRIDE_CVAR_INT(NodeCDO->PreviousNaniteVSMInvalidateOnLODDelta, TEXT("r.Nanite.VSMInvalidateOnLODDelta"), 1, bOverrideValues);
		}
