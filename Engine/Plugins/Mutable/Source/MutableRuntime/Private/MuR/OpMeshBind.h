...
namespace mu
{
			...
			const FMesh* BaseMesh, FShapeMeshTree& ShapeMeshTree, const FMeshBindColorChannelUsageMasks& ColorUsageMasks)	
	{
		...
		TArray<FReshapeVertexBindingData> BindData;
		{
			...
			MeshBufferIteratorConst<EMeshBufferFormat::Float32, float, 3> PositionIter(BaseMesh->GetVertexBuffers(), EMeshBufferSemantic::Position);
			TArrayView<const FVector3f> Positions = TArrayView<const FVector3f>(reinterpret_cast<const FVector3f*>(PositionIter.ptr()), BaseMesh->GetVertexCount());
			...
			if (NonRigidClusterIdx < 0)
			{
				...
				const int32 NumBatches = FMath::DivideAndRoundUp<int32>(VertexCount, NumBatchElems);

				TArrayView<const FPackedNormal> PackedNormals;
				if (ItNormalBase.ptr())
				{
					PackedNormals = TArrayView<const FPackedNormal>(reinterpret_cast<const FPackedNormal*>(ItNormalBase.ptr()), VertexCount);
				}

				auto ProcessVertexBatch = [
					Positions, PackedNormals, ItColorBase, &ShapeMeshTree, &BindData, BindValidityTolerance, &ColorUsageMasks, VertexCount, NumBatchElems
				] (int32 BatchId)
					{
						const int32 BatchBeginVertexIndex = BatchId * NumBatchElems;
						const int32 BatchEndVertexIndex = FMath::Min(BatchBeginVertexIndex + NumBatchElems, VertexCount);

						for (int32 VertexIndex = BatchBeginVertexIndex; VertexIndex < BatchEndVertexIndex; ++VertexIndex)
						{
							const FVector3f VertexNormal = PackedNormals.Num() ? PackedNormals[VertexIndex].ToFVector3f() : FVector3f::ZAxisVector;
							const float MaskWeight = ItColorBase.ptr() ? GetVertexMaskWeight(ItColorBase + VertexIndex, ColorUsageMasks) : 1.0f;

							BindReshapePoint(ShapeMeshTree, Positions[VertexIndex], VertexNormal, MaskWeight, BindData[VertexIndex], BindValidityTolerance);
						}
					};
			...
			bool& bOutSuccess)
    {
...

		{
			...
			if (ItIndices.GetFormat() == EMeshBufferFormat::UInt32 || ItIndices.GetFormat() == EMeshBufferFormat::Int32)
			{
				ShapeMeshDescriptor.Triangles = TArrayView<const UE::Geometry::FIndex3i>(reinterpret_cast<const UE::Geometry::FIndex3i*>(ItIndices.ptr()), ShapeTriangleCount);
			}
			else
			{
				...
				ShapeMeshDescriptor.Triangles = TArrayView<const UE::Geometry::FIndex3i>(FormattedTriangles);
			}
		}
