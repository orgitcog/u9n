...
void UWeightToolTransferManager::SetSourceMesh(USkeletalMesh* InSkeletalMesh)
{
	...
	const FMeshDescription* MeshDescription =  UE::ToolTarget::GetMeshDescription(SourceTarget, SourceParams);
	...
	, OriginalMeshDescription(InMeshDescription)
{
	...
	// if the mesh doesn't have any unused or non-manifold vertices, then we can skip making a cleaned duplicate and just use it directly
	// (the getters will return the original mesh)
	const FNonManifoldMappingSupport OriginalNonManifoldMapping(OriginalDynamicMesh);
	if (!OriginalDynamicMesh.HasUnusedVertices() && !OriginalNonManifoldMapping.IsNonManifoldVertexInSource())
	{
		return;
	}
