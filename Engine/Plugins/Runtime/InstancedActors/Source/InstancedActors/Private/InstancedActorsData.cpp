...
FInstancedActorsInstanceHandle UInstancedActorsData::AddInstance(const FTransform& Transform, const bool bWorldSpace)
{
	...
	// Add a new instance
	else if (!ensureMsgf(InstanceTransforms.Num() < (MAX_uint16 - 1), TEXT("Instance Actors do not support more than 65534 instances per cell. Make the cells smaller or lower the density of actors."))) // TODO: make this a constant in InstancedActorsIndex
	{
		return FInstancedActorsInstanceHandle();
	}
	else
