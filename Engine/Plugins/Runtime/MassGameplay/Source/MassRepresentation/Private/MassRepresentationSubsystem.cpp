...
void UMassRepresentationSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	if (UWorld* World = GetWorld())
	{
		...
		if (Visualizer == nullptr)
		{
			...
			// bind to processing phase events only if VisualizationComponent gets set - the phase event
			// handlers required that to be set, so no point in binding if it's not true
