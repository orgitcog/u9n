...
public:
	 ...
	 * Range 0.0 (mouth closed) to 1.0 (mouth fully open).
	 */
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Live2D Cubism", meta=(ClampMin = "0.0", ClampMax = "1.0", SliderMin = "0.0", SliderMax = "1.0", ToolTip = "Current lip sync value from 0.0 (closed) to 1.0 (open)."))
	float Value = 0.0f;
	 ...
	 * When enabled, uses audio envelope from Source. When disabled, manually set Value.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Live2D Cubism", meta=(ToolTip = "Enable automatic lip sync from audio source. Disable for manual control."))
	 ...
	 * Higher values make the mouth open more for quieter audio.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Live2D Cubism", meta=(EditCondition = "!bAutoEnabled", ClampMin = "1.0", ClampMax = "10.0", SliderMin = "1.0", SliderMax = "10.0", ToolTip = "Multiplier for lip sync sensitivity. Higher values = more mouth movement."))
	float Gain = 1.0f;
